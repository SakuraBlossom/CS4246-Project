...
class Render():
    ...
    def drawGrid(self):
        for j, column in enumerate(range(0, self.WINDOW_WIDTH, self.blockSize)):
            for i, row in enumerate(range(0, self.WINDOW_HEIGHT, self.blockSize)):
                if(self.state[i][j] < -1):
                    pygame.draw.rect(self.SCREEN, (255,0,0), [column,row,self.blockSize,self.blockSize])
                    print("TEST")
                if(self.state[i][j]==-1):
